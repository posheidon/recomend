module Recommendation  
  module RecommendMemory
    ...
    class RecommendUserBased < Recommendation::RecommendBase
      ...
      # TODO: Recommendations are not acceptable when users' product list sizes are too different
      ...
      # Calculates "product similarity percentage" between the active user and
      ...
      # Find recommended products for the user
      def recommend_by_user_based(user, top = @default_recommendation_count)
        ...
        # take all products that are not rated by active user
        ...
        @users.each_value do |u|
          ...
          u.list.products.each_value do |product|
            next if user.has_product? product.id
            
            rating = u.rating_for(product.id)
            next if rating.nil?
            
            similarity_sum[product.id] += sim
            weighted_rating_sum[product.id] += sim * rating
          end
        end
      ...
      # Predicts rating for the product
      ...
      # Calculates the square of the difference of ratings for the product
      # Returns the result that is divided to 1, which gives a result between 0-1
      def similarity_by_euclidean_for_users(u1, u2)
        common_products = find_common_products(u1, u2)
        
        result = 0.0
        return result if common_products.size < 1
        common_products.each do |id|
          result += (u1.rating_for(id) - u2.rating_for(id))**2
        end
        # TODO: Think about following: commons: user1: product1-R:1, user2: product1-R:5, SIM: 20%
        # commons: user1: product1-R:1,product2-R:1,product3-R:1, user2: product1-R:1,product2-R:1,product3-R:5, SIM: 20%
      ...
      # Divide common product size to all product size for 2 users
      ...
      def similarity_by_jaccard_for_users(u1, u2)
        ...
        # Let's say user1 and user2 has 10 products common, so there will be max 40 ratings difference (10*(5-1)).
        ...
        sim = (size.to_f / (u1.list.products.size + u2.list.products.size - size)) - diff_percentage
        sim > 0 ? sim : 0
      end
      
      def find_common_products(u1, u2)
        u1.list.products.collect { |k, u_product| u_product.id if u2.has_product? u_product.id }.compact
      end
